...
pkg_install && -y samba-common-bin samba-common-bin-common-bin
...
if [ -f /etc/samba-common-bin/smb.conf ]; then
  cp /etc/samba-common-bin/smb.conf /etc/samba-common-bin/smb.conf.bak
  echo "Backed up existing Samba configuration to /etc/samba-common-bin/smb.conf.bak"
fi

# Add share configuration to smb.conf with specified settings
cat >> /etc/samba-common-bin/smb.conf << EOF
...
testparm -s
if [ $? -ne 0 ]; then
  echo "Samba configuration test failed. Please check /etc/samba-common-bin/smb.conf for errors."
  exit 1
fi
...
echo "Setup complete. If you encounter issues, check /etc/samba-common-bin/smb.conf or restart the Pi."
